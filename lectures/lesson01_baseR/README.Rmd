...
```{r}
family <- c("Andrew", "Jacob", "Ryan", "Gary", "Melissa")
```
 
 - Create a vector `birth` with their birth year 
 
```{r}
birth <- c(1997, 2002, 2005, 1971, 1972)
```
...
```
 ...
 - Use inline code `r ##` to print your name (using indexing)
 
My name is `r family[1]`
 ...
 - What happens when:
   ...
   It says "named numeric(0)"
   
   + You assign a number to position 20 of the age vector? 
   
   When I call it, it says [1] NA
   I think I did this right. I don't believe we did this in class.
